...
(function (window) {
    ...
    var tools = (function () {
        ...
        var formatDate = function (oDate, sFormat) { // 入参：date对象，格式化格式
            ...
            if (/(y+)/.test(sFormat)) sFormat = sFormat.replace(RegExp.$1, (oDate.getFullYear() + "").substr(
                4 - RegExp.$1.length));
            for (var k in o)
                if (new RegExp("(" + k + ")").test(sFormat)) sFormat = sFormat.replace(RegExp.$1, (
    ...
    var ajax = (function () {
        var ajax = function (type, url, params, cb_success, cb_fail) {
            ...
            var contentType = type === 'POSTFORM' ? ' application/x-www-form-urlencoded' :
                'application/json';
                    ...
                    if (data.status == -1 && data.code == "402") { // token失效或者过期，会返回-1
                        window.top.Vue.prototype.$confirm('登录信息失效，请重新登录', '提示', {
                            ...
                            callback: function (action) {
                                if (action === 'confirm') {
                                    window.top.location.href = jasTools.base.rootPath +
                                        "/jasmvvm/pages/page-login/login.html";
                                }
                            }
                        });
                        return;
